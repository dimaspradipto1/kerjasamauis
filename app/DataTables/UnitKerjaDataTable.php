    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('nama_unit_kerja', function ($item) {
                return '<span class="text-primary">' . e($item->nama_unit_kerja) . '</span>';
            })
            ...
            ->addColumn('action', function ($item) {
                ...
                $btn .= '<a href="' . route('unit-kerja.edit', $item->id) . '" class="btn btn-sm btn-warning text-white rounded shadow-sm d-flex align-items-center justify-content-center" style="width: 30px; height: 30px;" title="Edit"><i class="bi bi-pencil-square" style="font-size: 13px;"></i></a>';
                $btn .= '<form action="' . route('unit-kerja.destroy', $item->id) . '" method="POST" class="m-0">' . csrf_field() . method_field('DELETE') . '<button type="submit" class="btn btn-danger btn-sm rounded shadow-sm d-flex align-items-center justify-content-center" style="width: 30px; height: 30px;" title="Hapus" onclick="return confirm(\'Yakin ingin menghapus data ini?\')"><i class="bi bi-trash3-fill" style="font-size: 13px;"></i></button></form>';
                $btn .= '</div>';
                return $btn;
            })
            ->rawColumns(['checkbox', 'action', 'nama_unit_kerja', 'keterangan']);
    }

    public function query(UnitKerja $model): QueryBuilder
    {
        return $model->newQuery()->orderBy('nama_unit_kerja', 'asc');
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('unit-kerja-table')
